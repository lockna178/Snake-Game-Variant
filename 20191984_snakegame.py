'''
Source Code - https://drive.google.com/file/d/1emT36o2VQTlETRcKjfduQGutqOcDhT_e/view
    Changes made:
    -added background music, used two channels to play multiple sound files at once
    -added button sound effects, game sound effects, losing sound effects
    -changed coloring to pastel tones
    -added the option to use WASD keys as well as arrow keys
    -changed some of the fonts / text size

Emma Rose Rorick // 20191984 // Visual Media Programming
'''
...
pg.mixer.init()
pg.init()
bg_music = pg.mixer.Sound("bgmusic.wav")  #source: https://youtu.be/RYsDvuSd-OA (royalty free)
chan1 = pg.mixer.find_channel()
chan1.set_volume(0.6)
chan1.queue(bg_music)


def m1(self):
    pg.mixer.init()
    pg.init()
    pg.mixer.music.load("boost.wav")
...
def m2(self):
    pg.mixer.init()
    pg.init()
    pg.mixer.music.load("nohealth.wav")
    pg.mixer.music.play()
    time.sleep(0.8)
...
def buttonsound(self):
    pg.mixer.init()
    pg.init()
    pg.mixer.music.load("buttonsound.wav")
    pg.mixer.music.play()
    time.sleep(0.2)
...
class game():
    ...
    def over(self):
        ...
        while 1:
            ...
            txts = pg.font.SysFont('Courier New',40).render('Game Over( •_•)   Score:',True,(255,255,255))
            ...
            self.screen.blit(txts,txtrect)
            pg.display.update()
            self.make_button((153,300,100,50),'Restart ;)', [(253,202,225),(205,156,178)], action = lambda:restart())
            if self.hover ==True:
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        txts = pg.font.SysFont('Impact',textsize).render(text,True,(0,0,0))
        txtrect = txts.get_rect()
    ...
    def loop(self):
        ...
        while self.game_over != True:
            self.screen.fill((116,167,228)) #bg
            self.snake.update()
            for x in self.blocks:
...
class startmenu():
    def __init__(self):
        self.screen = pg.display.set_mode((800,450))
        self.b1 = '(150, 300,100,50),"Start", [(119,221,119), (76,173,76)], action = self.start'
        self.b2 = '(550, 300,100,50),"Exit", [(173,76,76), (156,65,65)], action = self.exit'
    ...
    def mainloop(self):
        while 1:
            ...
            pg.display.update()
    def start(self):
        self.b1 = '(150, 300,100,50),"Normal", [(119,221,119), (76,173,76)], action = self.start3'
        self.b2 = '(550, 300,100,50),"Big", [(119,221,119), (76,173,76)], action = self.start4'
        self.buttons = [self.b1,self.b2]
    def start3(self):
        self.b1 = '(150, 300,100,50),"Easy", [(240,203,219), (131,99,113)], action = self.e'
        self.b2 = '(283, 300,100,50),"Normal", [(240,203,219), (131,99,113)], self.n'
        self.b3 = '(417, 300,100,50),"Hard", [(240,203,219), (131,99,113)], action = self.h'
        self.b4 = '(550, 300,100,50),"Expert", [(240,203,219), (131,99,113)], action = self.ex'
